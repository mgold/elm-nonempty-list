...
Elm.List.Nonempty.make = function (_elm) {
   ...
   if (_elm.List.Nonempty.values)
   return _elm.List.Nonempty.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "List.Nonempty",
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm);
   var length = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "Nonempty":
...
Elm.Random.Bool.make = function (_elm) {
   ...
   if (_elm.Random.Bool.values)
   return _elm.Random.Bool.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Random.Bool",
   $Random = Elm.Random.make(_elm),
   $Random$Extra = Elm.Random.Extra.make(_elm);
   var bool = A2($Random$Extra.frequency,
   _L.fromArray([{ctor: "_Tuple2"
                 ,_0: 1
                 ,_1: $Random$Extra.constant(true)}
                ,{ctor: "_Tuple2"
                 ,_0: 1
                 ,_1: $Random$Extra.constant(false)}]),
   $Random$Extra.constant(true));
   _elm.Random.Bool.values = {_op: _op
                             ,bool: bool};
   return _elm.Random.Bool.values;
};
Elm.Random = Elm.Random || {};
...
Elm.Random.List.make = function (_elm) {
   ...
   if (_elm.Random.List.values)
   return _elm.Random.List.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Random.List",
   $Random = Elm.Random.make(_elm),
   $Random$Extra = Elm.Random.Extra.make(_elm);
   ...
   var emptyList = $Random$Extra.constant(_L.fromArray([]));
   _elm.Random.List.values = {_op: _op
                             ,emptyList: emptyList
...
Elm.Random.Order.make = function (_elm) {
   ...
   if (_elm.Random.Order.values)
   return _elm.Random.Order.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Random.Order",
   $Basics = Elm.Basics.make(_elm),
   $Random = Elm.Random.make(_elm),
   $Random$Extra = Elm.Random.Extra.make(_elm);
   var order = A2($Random$Extra.selectWithDefault,
   $Basics.EQ,
   _L.fromArray([$Basics.LT
                ,$Basics.EQ
                ,$Basics.GT]));
   _elm.Random.Order.values = {_op: _op
                              ,order: order};
   return _elm.Random.Order.values;
};
Elm.Random = Elm.Random || {};
...
Elm.Test.make = function (_elm) {
   ...
   $Check = Elm.Check.make(_elm),
   $Check$Investigator = Elm.Check.Investigator.make(_elm),
   $Check$Runner$Browser = Elm.Check.Runner.Browser.make(_elm),
   $List = Elm.List.make(_elm),
   $List$Nonempty = Elm.List.Nonempty.make(_elm),
   $Maybe = Elm.Maybe.make(_elm);
   var nonemptylist = function (elem) {
      return $Check$Investigator.tuple({ctor: "_Tuple2"
                                       ,_0: elem
                                       ,_1: $Check$Investigator.list(elem)});
   };
   var testSuite = A2($Check.suite,
   "Nonempty List Test Suite",
   _L.fromArray([A2($Check.$for,
                A2($Check.$true,
                $Check.claim("dropping tail makes singleton"),
                function (_v0) {
                   return function () {
                      switch (_v0.ctor)
                      {case "_Tuple2":
                         return $List$Nonempty.isSingleton($List$Nonempty.dropTail(A2($List$Nonempty.Nonempty,
                           _v0._0,
                           _v0._1)));}
                      _U.badCase($moduleName,
                      "on line 16, column 21 to 66");
                   }();
                }),
                ...
                A2($Check.is,
                A2($Check.that,
                $Check.claim("converting to and from a normal list is the identity"),
                function (_v4) {
                   return function () {
                      switch (_v4.ctor)
                      {case "_Tuple2":
                         return $List$Nonempty.fromList($List$Nonempty.toList(A2($List$Nonempty.Nonempty,
                           _v4._0,
                           _v4._1)));}
                      _U.badCase($moduleName,
                      "on line 22, column 21 to 65");
                   }();
                }),
                function (_v8) {
                   return function () {
                      switch (_v8.ctor)
                      {case "_Tuple2":
                         return $Maybe.Just(A2($List$Nonempty.Nonempty,
                           _v8._0,
                           _v8._1));}
                      _U.badCase($moduleName,
                      "on line 24, column 21 to 43");
                   }();
                }),
                nonemptylist($Check$Investigator.$int))
                ,A2($Check.$for,
                A2($Check.is,
                A2($Check.that,
                $Check.claim("length is 1 more than `length tail`"),
                function (_v12) {
                   return function () {
                      switch (_v12.ctor)
                      {case "_Tuple2":
                         return $List$Nonempty.length(A2($List$Nonempty.Nonempty,
                           _v12._0,
                           _v12._1));}
                      _U.badCase($moduleName,
                      "on line 30, column 21 to 50");
                   }();
                }),
                function (_v16) {
                   return function () {
                      switch (_v16.ctor)
                      {case "_Tuple2":
                         return $List.length(_v16._1) + 1;}
                      _U.badCase($moduleName,
                      "on line 32, column 21 to 39");
                   }();
                }),
                nonemptylist($Check$Investigator.$int))
                ,A2($Check.$for,
                ...
                function (_v20) {
                   return function () {
                      ...
                      {case "_Tuple2":
                         switch (_v20._1.ctor)
                           {case "_Tuple2":
                              return $List$Nonempty.toList(A2($List$Nonempty._op[":::"],
                                _v20._0,
                   ...
                   return function () {
                      ...
                      {case "_Tuple2":
                         switch (_v26._1.ctor)
                           ...
                           break;}
                      _U.badCase($moduleName,
                      "on line 40, column 26 to 38");
                   }();
                }),
                $Check$Investigator.tuple({ctor: "_Tuple2"
                                          ,_0: $Check$Investigator.$int
                                          ,_1: nonemptylist($Check$Investigator.$int)}))
                ,A2($Check.$for,
                A2($Check.$true,
                $Check.claim("fromElement results in a singleton"),
                function (x) {
                   return $List$Nonempty.isSingleton($List$Nonempty.fromElement(x));
                }),
                $Check$Investigator.$int)
                ,A2($Check.$for,
                A2($Check.$true,
                $Check.claim("fromList fails only for the empty List"),
                function (xs) {
                   return function () {
                      var _v32 = $List$Nonempty.fromList(xs);
                      switch (_v32.ctor)
                      {case "Just":
                         return $Basics.not($List.isEmpty(xs));
                         case "Nothing":
                ...
                $Check$Investigator.list($Check$Investigator.$int))
                ,A2($Check.$for,
                A2($Check.is,
                A2($Check.that,
                $Check.claim("map then toList == List.map"),
                function (_v34) {
                   return function () {
                      switch (_v34.ctor)
                      {case "_Tuple2":
                         return $List$Nonempty.toList($List$Nonempty.map(F2(function (x,
                           y) {
                              return x * y;
                           })(2))(A2($List$Nonempty.Nonempty,
                           _v34._0,
                           _v34._1)));}
                      _U.badCase($moduleName,
                      "on line 61, column 21 to 68");
                   }();
                }),
                function (_v38) {
                   return function () {
                ...
                nonemptylist($Check$Investigator.$int))]));
   var result = $Check.quickCheck(testSuite);
   var main = $Check$Runner$Browser.display(result);
